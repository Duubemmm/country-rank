...
const regionList = ["Americas", "Antarctic", "Africa", "Asia", "Europe", "Oceania"];
...
const CountryRanking = ({ className = "" }) => {
  ...
  // --- STEP 3: DERIVING DISPLAY DATA WITH useMemo ---
  ...
  // --- STEP 4: EVENT HANDLERS FOR FILTERS ---
  ...
  return (
    <section className={`bg-[#1B1D1F] text-white shadow-2xl rounded-lg w-full max-w-6xl mx-auto flex flex-col p-4 sm:p-6 ${className}`}>
          ...
          <div className="flex items-center gap-x-6">
            <div className="flex items-center gap-x-2">
              <input
                ...
                onChange={handleFilterChange}
                className="h-4 w-4 rounded bg-[#282B30] border-gray-500 text-cyan-500 focus:ring-cyan-500"
              />
              <label htmlFor="unMember" className="text-sm text-gray-300 cursor-pointer">
                Member of UN
              </label>
            </div>
            <div className="flex items-center gap-x-2">
              <input
                ...
                className="h-4 w-4 rounded bg-[#282B30] border-gray-500 text-cyan-500 focus:ring-cyan-500"
              />
              <label htmlFor="independent" className="text-sm text-gray-300 cursor-pointer">
                Independent
              </label>
            </div>
          </div>
      ...
      <div className="flex-grow overflow-auto rounded-lg">
        {status === 'loading' && <p className="text-center p-8">Loading countries...</p>}
        {status === 'failed' && <p className="text-center text-red-500 p-8">{error}</p>}
        {status === 'succeeded' && (
          <table className="w-full text-left text-sm table-auto">
            <caption className="sr-only">A table of countries sorted by population, with details on area and region.</caption>
            <thead className="bg-[#282B30] sticky top-0 z-10">
              <tr>
                <th scope="col" className="p-3 w-[5%] text-center font-semibold">#</th>
                <th scope="col" className="p-3 w-[5%] font-semibold">Flag</th>
                <th scope="col" className="p-3 w-[30%] font-semibold">Name</th>
                <th scope="col" className="p-3 w-[20%] font-semibold">Population</th>
                <th scope="col" className="p-3 w-[20%] font-semibold">Area ($km^2$)</th>
                <th scope="col" className="p-3 w-[20%] font-semibold">Region</th>
              </tr>
            </thead>
            <tbody>
              {displayCountries.length > 0 ? (
                displayCountries.map((country, index) => (
                  ...
                  >
                    ...
                    <td className="p-3 font-mono text-cyan-400">{formatNumber(country.population)}</td>
                    <td className="p-3 font-mono">{formatNumber(country.area)}</td>
